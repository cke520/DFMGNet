...
class Cross_Mamba_Attention_SSM(nn.Module):
    ...
    def forward(self, x_1: torch.Tensor, x_2: torch.Tensor):
        ...
        #selective_scan = selective_scan_fn
        #import inspect
        #print(inspect.getfile(selective_scan_fn))
        #exit()
        B, L, d = x_1.shape
        x_1 = x_1.permute(0, 2, 1)
        x_2 = x_2.permute(0, 2, 1)
        x_dbl_1 = self.x_proj_1(rearrange(x_1, "b d l -> (b l) d"))  # (bl d)
        x_dbl_2 = self.x_proj_2(rearrange(x_2, "b d l -> (b l) d"))  # (bl d)
        ...
        #print(x_rgb.shape, dt_rgb.shape,A_rgb.shape, B_rgb.shape, C_e.shape, self.D_2.float().shape)
        #### new C###
        # 计算 C_1 的均值和标准差
        mean_C1 = torch.mean(C_1)
        std_C1 = torch.std(C_1)  # 直接使用 torch.std() 计算标准差
        C_1_g = (C_1 - mean_C1) / std_C1  # 标准化：(x - mean) / std

        # 计算 C_2 的均值和标准差
        mean_C2 = torch.mean(C_2)
        std_C2 = torch.std(C_2)  # 直接使用 torch.std() 计算标准差
        C_2_g = (C_2 - mean_C2) / std_C2  # 标准化：(x - mean) / std


        C = 0.5*C_1_g + 0.5*C_2_g


        y_1 = selective_scan(
            x_1, dt_1,
            A_1, B_1, C+C_1, self.D_1.float(),      # self.D_1.float()
            delta_bias=self.dt_proj_1.bias.float(),
            delta_softplus=True,
        )
        y_2 = selective_scan(
            x_2, dt_2,
            A_2, B_2, C+C_2, self.D_2.float(),         #self.D_2.float(),
            delta_bias=self.dt_proj_2.bias.float(),
            delta_softplus=True,
        )
        # assert out_y.dtype == torch.float
        y_1 = rearrange(y_1, "b d l -> b l d")
        y_1 = self.out_norm_1(y_1)
        y_2 = rearrange(y_2, "b d l -> b l d")
        y_2 = self.out_norm_2(y_2)
...
class Cross_Mamba_Fusion_SSM(nn.Module):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
        ...
        factory_kwargs = {"device": None, "dtype": torch.float32}
        super().__init__()
        self.d_model = d_model
        self.d_state = math.ceil(self.d_model / 6) if d_state == "auto" else d_state  # 20240109
        self.expand = ssm_ratio
        ...
        self.x_proj_cat = nn.Linear(self.d_inner*2, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs)

        self.dt_proj_1 = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                    **factory_kwargs)
        self.dt_proj_2 = self.dt_init(self.dt_rank, self.d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                    **factory_kwargs)
        self.dt_proj_cat = self.dt_init(self.dt_rank*2, self.d_inner*2, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                                    **factory_kwargs)

        # A, D =======================================
        self.A_log_1 = self.A_log_init(self.d_state, self.d_inner*2)  # (D, N)
        self.D_1 = self.D_init(self.d_inner*2)  # (D)


        # out norm ===================================
        self.out_norm_1 = nn.LayerNorm(self.d_inner*2)
    ...
    def forward(self, x_1: torch.Tensor, x_2: torch.Tensor):
        ...
        fused_x =  torch.cat([x_1 , x_2], dim=2)


        B, L, d = fused_x.shape

        #### x_1 preperation ####
        fused_x = fused_x.permute(0, 2, 1).contiguous()
        
        x_dbl = self.x_proj_cat(rearrange(fused_x, "b d l -> (b l) d"))  # (bl d)
        #print( ' x_dbl',x_dbl.dtype )
        dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
        B = rearrange(B, "(b l) dstate -> b dstate l", l=L).contiguous()
        C = rearrange(C, "(b l) dstate -> b dstate l", l=L).contiguous()

        B =  B.to(fused_x.dtype) 
        C =  C.to(fused_x.dtype) 
        _, L, d = x_1.shape
        ...
        A = -torch.exp(self.A_log_1.float())  # (k * d, d_state)
        ...
        #B_cat = torch.cat((B_1,B_2), dim = 1)
        #C_cat = torch.cat((C_1,C_2), dim = 1)

        dt_cat = torch.cat((dt_1,dt_2), dim = 1)
        dt_cat = self.dt_proj_cat.weight @ dt_cat.t()
        dt_cat = rearrange(dt_cat, "d (b l) -> b d l", l=L)  
        dt_cat =  dt_cat.to(fused_x.dtype) 
        #print(A.dtype, B.dtype,B.dtype,dt_cat.dtype,)
        #exit()


        y = selective_scan(
            fused_x, dt_cat,
            A, B, C, self.D_1.float(),
            delta_bias=self.dt_proj_cat.bias.float(),
            delta_softplus=True,
        )
...
###############################
class Mamba_SS2D_SSM(nn.Module):
    '''
    Mamba Attention Selective Scan 2D Module with SSM
    '''
    def __init__(
        self,
        # basic dims ===========
        ...
        **kwargs,
        ...
        self.softmax_version = softmax_version
        self.d_model = d_model
        self.d_state = math.ceil(self.d_model / 6) if d_state == "auto" else d_state   # 20240801
        self.d_conv = d_conv
        self.expand = ssm_ratio
        self.d_inner = int(self.expand * self.d_model)
        self.dt_rank = math.ceil(self.d_model / 16) if dt_rank == "auto" else dt_rank
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            y = self.mb_ssm(x_conv) 
 
            # y +=   x_conv

            #y_fuse = self.CMF_ssm(y_rgb, y_e) 
      
            # to b, d, h, w
            y = y.view(B, H, W, -1)
            #print(y_rgb.shape, y_e.shape, y_fuse.shape)
            #fuse = torch.cat([y_rgb, y_e, y_fuse], dim=-1)

        out = self.dropout(self.out_proj(y))
        out = out.permute(0,  3, 1,  2).contiguous()
        #fuse = torch.cat([out_rgb, out_e, out], dim=-1)
...
class CrossMambaFusion_SS2D_SSM(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.CMF_ssm = Cross_Mamba_Fusion_SSM(
            d_model=self.d_model,
            d_state=self.d_state,
            ssm_ratio=ssm_ratio,
            dt_rank=dt_rank,
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        if self.d_conv > 1:
            ...
            y_fuse  = y_fuse.view(B, H, W, -1)
            #print(y_rgb.shape, y_e.shape, y_fuse.shape)
            fuse = torch.cat([y_rgb, y_e, y_fuse], dim=-1)
       
        out = self.dropout(self.out_proj(fuse))
        #print(out.shape)
...
class MambaRontingBlock(nn.Module):
    '''
    Mamba Ronting Block, with 2d SSM
    '''
    def __init__(
        self,
        hidden_dim: int = 0,
    ...
    def _forward(self, x: torch.Tensor):   #B, H, W, C

        x_mb= self.op(x)     ##B, H, W, C

       # x_rgb = x_rgb + self.drop_path1(x_rgb_cross)
        #x_e = x_e + self.drop_path2(x_e_cross)
        x_mb = x + self.drop_path(x_mb)
        x_mb  = x_mb.permute(0, 3, 1, 2 ).contiguous()
        return  x_mb
...
class CrossMambaFusionBlock(nn.Module):
    ...
    def _forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):   #B, H, W, C

        x_fuse_cross = self.op(x_rgb, x_e)     ##B, H, W, C

       # x_rgb = x_rgb + self.drop_path1(x_rgb_cross)
        #x_e = x_e + self.drop_path2(x_e_cross)
        x_fuse = x_rgb + x_e + self.drop_path(x_fuse_cross)
